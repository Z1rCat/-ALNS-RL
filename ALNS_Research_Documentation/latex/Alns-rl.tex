...
\section{任务背景：在 RL 回报曲线中显式标注分布变化}
...
\item \textbf{动作空间}：
\begin{itemize}
\item $a=0$：等待 / 保持当前方案（Wait）；
\item $a=1$：触发重规划（Reroute）。
\end{itemize}
\item \textbf{课程式跳级}：当 RL 达到阶段性收敛条件后，不必按 $0\ldots499$ 的顺序跑完整个环境序列，而是根据场景结构跳转到下一个阶段或直接进入测试；
\item \textbf{物理隔离（\texttt{run\_dir}）}：每一次运行都输出到独立目录
\texttt{codes/logs/run\_*/}，包含 \texttt{data/}、日志 CSV 与元配置文件，用于复现实验与对比分析。
\end{itemize}

本次改动主要是基于上一次老师提出的关键意见：
在最终绘制 RL 的reward曲线时，把环境的分布切换也明确展示出来，否则看不出来不同分布对RL的影响。


基于这一意见，进行了一系列实现。主要围绕
\textbf{“分布漂移如何被正确展示、以及这暴露了哪些原有机制的问题”}展开。

\section{实现可视化后暴露的问题：RL 可能“过早收敛”}

在将分布切换显式画入 回报曲线之后，我发现了一个此前被忽略的重要漏洞。

原有的智能跳级逻辑是：
\begin{itemize}
\item 只要 RL 的 回报的滑动平均 达到阈值，就认为当前阶段已经学会；
\item 一旦满足该条件，就允许直接进入测试阶段。
\end{itemize}

这一逻辑在 \textbf{单阶段或 分布外 设计} 中是合理的，
...
每一个 Excel 环境文件都包含一个 \texttt{\_\_meta\_\_} 工作表，至少记录：
...
RL 在构建状态时读取该工作表，并将对应信息写入日志。

\section{关键改动二：基于时序结构特性的六类分布设计与能力假设}

为了全面评估 RL 在动态环境下的鲁棒性与适应性，我们基于 \texttt{distribution\_config.json} 构建了六类典型的分布模式。

这里的关键做法是：\textbf{将 500 个环境实例的时序生成逻辑与 $0\ldots349$（训练集）/ $350\ldots499$（测试集）的切分点进行精确对齐}。通过控制分布切换点相对于测试切分点的位置，我们为每一类场景赋予了明确的“能力探针”语义。
...
\toprule
\textbf{类别} & \textbf{模式} & \textbf{时序结构 （索引 $0 \to 499$）} & \textbf{核心验证目标 （假设）} \\
\midrule
...
\hline
\textbf{泛化} (S3) & \texttt{ab} & \textbf{分布阶跃} \newline A: $0\ldots349$ （训练） \newline B: $350\ldots499$ （测试） & \textbf{分布外 零样本泛化}：训练阶段从未见过分布 B。验证当环境发生结构性突变时，RL 是否具备基本的通用性，还是会因过拟合 A 而彻底失效。 \\
\hline
...
\begin{enumerate}
    \item \textbf{分布外 的绝对隔离 (S3\_1 \texttt{ab})：} 
    ...
    设计 $A(0\ldots99) \to B$ 的初衷是制造一个“收敛陷阱”。传统的 提前停止 往往会在 索引 $50\ldots80$ 左右因为 回报趋稳而触发停止，从而完全错过后续的 B 分布。该模式强迫系统必须引入\textbf{基于环境特征的跳级检查}，否则训练出的模型在测试集（全为 B）将表现极差。
...
\item 分布外 场景（S3）：训练只跑 A，达标后直接进入测试；
...
在强化学习模型的评估中，单纯依赖平均回报往往难以全面衡量智能体的真实能力。例如，在低干扰环境（如对数正态分布均值为 3 的场景）中，简单的“始终等待（Always-Wait）”策略即可获得理论最优解。若 RL 仅获得高分，无法证明其具备了状态感知与复杂决策能力。

因此，为了量化 RL 相对于启发式规则的智能程度，我们构建了包含三种基准策略：Always-Wait、Always-Reroute、Random的对比体系。同时，针对本框架特有的跳级机制导致的训练序列非线性问题，设计并实现了基准策略回放机制，以确保在完全一致的上下文环境中进行公平对比。

\subsection{基准策略回放机制}

由于本研究引入了基于收敛阈值的跳级机制，RL 智能体 在训练和测试阶段经历的算例序列（\texttt{table\_number}）是动态变化的（例如，模型可能在 索引 100 快速收敛后直接跳跃至 索引 350）。传统的静态测试集评估法无法复现 RL 做出决策时的时序上下文。

为此，本研究提出了一种对齐轨迹的回放机制，通过脚本 \texttt{run\_benchmark\_replay.py} 实现评估：

\begin{enumerate}
    \item \textbf{环境复用}：回放过程直接读取 RL 运行时生成的沙盒目录（\texttt{run\_dir/data/}）下的 Excel 环境文件，严格禁止重新生成不确定性事件。这确保了基准策略与 RL 面对的是完全相同的“试卷”，消除了随机种子带来的环境噪音。
    \item \textbf{序列重放}：系统解析 RL 的运行日志 \texttt{rl\_trace.csv}，提取出 RL 实际经历的 $(table\_number, phase)$ 精确时序序列。基准策略将严格按照此顺序执行，包括相同的跳级点和倒序测试路径。
    \item \textbf{闭环同步}：基准脚本并非简单地离线计算，而是像 RL 智能体 一样与 ALNS 仿真器进行实时交互。它根据历史 Phase 设置仿真参数，发出固定的基准动作，并轮询等待 ALNS 返回真实的回报。这保证了底层仿真器的状态转移与 RL 运行时完全一致。
\end{enumerate}
...
\begin{itemize}
    ...
    \end{equation}
    以 50\% 的概率随机选择动作。用于排除环境自身的偶然性（例如某些死局下无论如何选择收益均相同），确立性能下界（下界），证明 RL 的优势来源于有效的特征学习而非运气。
\end{itemize}

\subsection{深度对比目标与累计优势指标}

通过将 RL 的性能曲线与上述基准策略进行同坐标系对比（参见实验结果部分的 图 4 等），本研究旨在回答以下三个核心问题：

\begin{enumerate}
    \item \textbf{有效性}：RL 是否在绝大多数时间步中击败了 Random 和表现较差的那个确定性基准？
    \item \textbf{状态敏感性}：RL 是否展现出了动态切换能力？即在低风险区逼近 Always-Wait 的表现，而在高风险区逼近 Always-Reroute 的表现。
    \item \textbf{分布适应性}：当环境发生分布漂移（如 分布外 场景）导致某一固定基准策略系统性崩溃时，RL 是否能保持鲁棒性？
\end{enumerate}

为了直观量化 RL 相对于基准策略的持续性收益，我们定义了**“累计优势”**指标 $\mathcal{V}_t$。该指标计算 RL 累积获得的奖励与基准策略累积奖励的差值，其定义如下：
...
为了从机制、现象到量化结果全方位验证 ALNS-RL 框架的有效性，我们升级了可视化脚本 \texttt{plot\_paper\_figure.py}。该系统针对每个实验 Run 生成四张核心图表，构成了完整的证据链。

以下以 S3\_1 场景（分布外 泛化测试：训练于常态分布，测试于未见过的瘫痪分布）为例进行说明：

\subsection{图 1：环境分布漂移全景图 }
\begin{figure}[H]
    \centering
...
\textbf{解读指南：}
\begin{itemize}
    \item \textbf{左子图（时序散点）}：展示了算例序列对应的环境压力。蓝色点群代表训练阶段（低压常态），红色点群代表测试阶段（高压瘫痪）。虚线清晰标记了分布发生结构性突变的时刻。
    \item \textbf{右子图（核密度估计 KDE）}：展示了环境参数的概率分布。双峰（或明显分离的单峰）形态证明了训练集与测试集存在显著的 \textbf{Covariate Shift}，确立了 分布外实验的物理基础。
\end{itemize}

\subsection{图 2：适应性 S 曲线 }
\begin{figure}[H]
    \centering
...
\textbf{解读指南：}
\begin{itemize}
    \item \textbf{双轴设计}：左轴为平滑奖励，右轴（黑色点状阶梯线）为环境真实均值。
    \item \textbf{核心看点}：
    \begin{enumerate}
        \item \textbf{突变响应}：当右轴黑线阶梯式上升（环境变难）时，观察蓝色 RL 曲线是否出现短暂下跌；
...
\textbf{解读指南：}
\begin{itemize}
    \item \textbf{图表含义}：展示了 RL 智能体 在不同阶段（Y轴：训练阶段 对比 \texttt{implement} 阶段）对四种细分动作（X轴）的选择频率。
    \item \textbf{机制证明}：这是解释 RL \textbf{“学到了什么”} 的关键。
    \item \textbf{预期现象}：在 S3\_1 中，训练阶段（低压）颜色应集中在 \texttt{Removal-Wait}（倾向保守）；而进入测试阶段（高压）后，高亮区域应显著转移至 \texttt{Removal-Reroute}（倾向激进）。这种 \textbf{策略漂移 } 证明了模型具备状态感知能力，而非盲目记忆。
\end{itemize}

\subsection{图 4：累计优势趋势图 }
\begin{figure}[H]
    \centering
...
面对“多分布模式 $\times$ 多算法类型 $\times$ 多请求规模（$R$） $\times$ 多基准策略”带来的组合爆炸式计算需求，我们构建了一套分层级、高鲁棒的自动化实验编排系统。该系统旨在实现从本地开发环境（MVP）到服务器高性能计算环境（HPC）的无缝迁移。

\subsection{分层调度体系}
为平衡调试灵活性与大规模计算效率，我们实现了三级调度脚本：
\begin{itemize}
    \item \textbf{公共调度内核 (\texttt{run\_experiments\_common.py})}：封装了实验配置解析、任务队列管理与异常捕获逻辑，作为核心库被上层调用。
    \item \textbf{本地调试入口 (\texttt{run\_experiments\_local.py})}：面向开发环境优化，默认采用小规模样本与低并发度，用于快速验证代码逻辑。
    \item \textbf{服务器全量入口 (\texttt{run\_experiments\_server.py})}：面向生产环境优化，支持全量参数矩阵的一键部署。
\end{itemize}

\subsection{原子任务全链路闭环}
系统将单次实验定义为一个不可分割的“原子任务单元”，每个单元严格执行以下标准化闭环流程：
\begin{enumerate}
    \item \textbf{主程序执行}：调用 \texttt{Dynamic\_master34959.py}，完成从环境生成、RL 模型训练到测试阶段的全过程，并将所有运行数据落盘至独立的沙盒目录。
    \item \textbf{反事实评估}：自动触发 \texttt{run\_benchmark\_replay.py --policy all}，在相同的环境序列下回放 Wait/Reroute/Random 等基准策略，生成对比数据。
    \item \textbf{自动化绘图}：调用 \texttt{plot\_paper\_figure.py}，基于生成的日志自动产出符合标准的四张核心图表，实现“无人值守”的证据生成。
\end{enumerate}
...
\begin{itemize}
    \item \textbf{沙盒隔离并发}：以“独立 Run ID 目录”为最小并发单元。不同实验进程在物理文件系统上完全隔离，彻底消除了写冲突风险。
...
环境结构（Fig1）、回报适应曲线（Fig2）、动作策略分布（Fig3）以及相对基准策略的累计优势（Fig4）。
...
\caption{DQN 在 V1\_3 场景下的 回报适应曲线（右轴为环境方差）}
\end{figure}

Fig2 表明，在方差显著增大的阶段，DQN 的 回报 并未出现系统性下降，
...
\caption{A2C 在 V1\_3 场景下的 回报适应曲线}
\end{figure}

与 DQN 类似，A2C 在该场景下的 回报 表现同样稳定，
...
% ============================================================
\subsection{S3\_1：中等均值漂移（分布外 泛化）}
% ============================================================
...
\caption{DQN 在 S3\_1 场景下的 回报 变化}
\end{figure}

Fig2 表明在分布切换发生后，DQN 的 回报 明显下降，
...
\caption{A2C 在 S3\_1 场景下的 回报适应曲线}
\end{figure}
...
\caption{DQN 在 S5\_1 场景下的 回报 变化}
\end{figure}

Fig2 显示在强分布切换发生后，DQN 的 回报 明显下探，
且几乎不存在后续恢复过程。
...
\caption{A2C 在 S5\_1 场景下的 回报 变化}
\end{figure}

A2C 在该场景下同样经历性能下降，
但 回报 并未完全失控，表现出一定的鲁棒性。
...
通过对 DQN 与 A2C 两种典型强化学习算法在不同分布漂移场景（S3\_1, S5\_1, V1\_3）下的系统性对比，结合基准策略回放机制生成的累计优势曲线，我们得到如下阶段性结论：

\subsection{1. 策略塌缩的双重性：从“最优收敛”到“灾难性过拟合”}
实验通过对比 V1\_3（方差突变）与 S3\_1/S5\_1（均值突变）揭示了“策略塌缩”现象的本质差异：

\begin{itemize}
    \item \textbf{良性塌缩}：在 V1\_3 场景（均值恒定为 15，仅方差增大）中，DQN 的策略完全塌缩为“等待（Wait）”（图 7），A2C 也高度倾向于等待（图 11）。结合累计优势图（图 8, 图 12）可见，此时 RL 表现与 Always-Wait 基准持平且优于 Always-Reroute。这表明：\textbf{在均值未发生结构性改变的场景下，单一动作往往就是全局最优解}。此时的策略塌缩是算法成功收敛到最优启发式规则的体现，而非失效。
    \item \textbf{恶性塌缩}：在 S3\_1 和 S5\_1 场景（均值从 9 突变至 90）中，DQN 依然塌缩在“等待”动作上（图 15, 图 23），导致 回报 断崖式下跌且无法恢复（图 14, 图 22）。与之形成鲜明对比的是，Always-Reroute 基准在测试段表现优异（图 16）。这证明 DQN \textbf{过拟合了训练阶段的低压环境特征}，丧失了探索新最优策略的能力。这种塌缩在分布发生结构性漂移时是致命的。
\end{itemize}

\subsection{2. 算法适应性差异机理：DQN 的刚性 对比. A2C 的柔性}
在应对强均值漂移（S3\_1, S5\_1）时，A2C 展现出了显著优于 DQN 的鲁棒性（图 18 对比 图 14）：

\begin{itemize}
    \item \textbf{DQN （价值型）}：由于依赖 $\epsilon$-贪心 策略进行探索，在训练后期 $\epsilon$ 衰减后，DQN 极易陷入局部极小值。当环境从“无需重规划”突变为“必须重规划”时，DQN 仍基于旧有的 Q 值表过高估计“等待”的价值，导致其无法感知新环境下的动作优势反转。
    \item \textbf{A2C (Actor-Critic)}：得益于其显式的策略建模和熵正则化，A2C 在测试阶段仍保留了非零的动作概率分布（图 19, 图 27 展示了 A2C 在高压区保留了约 15\%-20\% 的 Reroute 动作）。这种\textbf{策略的“柔性”}使其在环境突变后能够快速试错并捕捉到重规划带来的高额回报，从而实现了 S 型适应曲线的爬升。
\end{itemize}
...
\begin{itemize}
    \item \textbf{均值漂移是核心挑战}：环境均值的数量级变化（如 9 $\to$ 90）直接改变了最优策略的性质（从 Wait $\to$ Reroute）。这是导致模型泛化失败的主因，也是验证“适应性”的最有效试金石。
    \item \textbf{方差漂移主要影响风险偏好}：在 V1\_3 中，方差的剧增并未导致模型性能崩溃。这说明在期望值不变的情况下，当前的 RL 算法能够保持鲁棒，并未因环境噪声的增加而迷失方向。
\end{itemize}
...
\begin{itemize}
    ...
    \item 通过 图 16，我们清晰地看到 RL 跑输了 Always-Reroute，从而确凿地判定这是\textbf{算法适应性失败}，而非环境不可行。这一机制成功排除了环境噪音干扰，准确定位了算法缺陷。
\end{itemize}
